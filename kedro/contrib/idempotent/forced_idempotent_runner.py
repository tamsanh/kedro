...
def run_node_forced_idempotently(node: Node, catalog: DataCatalog, state: IdempotentStateStorage) -> Node:
...
class ForcedIdempotentSequentialRunner(IdempotentSequentialRunner):
    ...
    def _run(self, pipeline: Pipeline, catalog: DataCatalog) -> None:
        ...
        for exec_index, node in enumerate(nodes):
            try:
                run_node_forced_idempotently(node, catalog, self.state_storage)
                done_nodes.add(node)
            except Exception:
